...
//Since this is for testing, disable SSL checks DO NOT DO THIS IN A LIVE ENVIRONMENT	
...
else
{
    ...
    var secadminPassword = argv.s,
		...
		//Current sets 2 ranges, need to update to make this a parameter
	...
	//triggers SDS encryption on all machines with appropriate devices (/dev/sd*)
	...
	//Build array of securevm machiens with suitable SDS devices for encryption
	function listsvmsdsmachines()
	{
		process.stdout.write('Collecting device info from CloudLink Center..\n\n');
		return new Promise(function (resolve, reject) {
            req({'method': 'GET', 'uri': machinesUrl})
                .then(function (response) {
                    var parsed = JSON.parse(response);
                    for (var i = 0; i < parsed.length; ++i)
                    {
                        ...
                        for (var j = 0; j < parsed[i].resources.length; ++j)
                        {					
							// If the device has /dev/ in the name 
							var mount = parsed[i].resources[j].mpoint;
							var myindex = mount.indexOf('/dev/sd')
							if (myindex >= 0 ){
								var line = parsed[i].name;
								//Add machine and mount point to the array
								scaleio_svmlist.push(line+':'+mount+'!'+parsed[i].uuid+'*'+parsed[i].resources[j].id);
							}
						}  
                    }
				//Output array info
				process.stdout.write("Devices found:" + '\n');
				for (i=0;i<scaleio_svmlist.length;i++)
				{
					process.stdout.write(scaleio_svmlist[i] + '\n');
				}					
				resolve();
                })
                .catch(function (error) {
	...
	function processsdsdevices()
	{
		process.stdout.write('Running encryption requests on the device list..\n\n');
		return new Promise(function (resolve, reject) {
            req({'method': 'GET', 'uri': machinesUrl})
                .then(function (response) {
                    var parsed = JSON.parse(response);
					...
					// Array is in the form NAME:MOUNT!UUID*MOUNTID);
					
					
					for (i=0;i<scaleio_svmlist.length;i++)
					{
					
					//Get positions and save info
					arrayitem = scaleio_svmlist[i]
					colon = arrayitem.indexOf(":")
					exclamation = arrayitem.indexOf("!")
					star = arrayitem.indexOf("*")
					
					
					
					machinename = arrayitem.substring(0,colon)
					process.stdout.write('Machine is '+ machinename +'\n');
					
					mountpoint = arrayitem.substring(colon+1,exclamation)
					process.stdout.write('Mount is '+ mountpoint +'\n');
					
					uuidinfo = arrayitem.substring(exclamation+1,star)
					process.stdout.write('UUID is '+ uuidinfo +'\n');
					
					
					mountid = arrayitem.substring(star+1,arrayitem.length)
					process.stdout.write('Mount ID is '+ mountid +'\n');
					
										
					
							
					req({'method': 'PUT', 'uri': machinesUrl + '/'+uuidinfo +'/encryption/' + mountid, 'body': JSON.stringify(sdsdeviceBody)});
					process.stdout.write('Requested encryption of ' + mountid + ' on machine: ' + machinename + '\n');
